...
// VibeSynq AI endpoint plugin - extracted from vibesynqPlugin.ts
export const vibesynqAiPlugin = new Elysia({ name: "vibesynq-ai" }).post(
	"/api/ask-ai",
	async ({ body, set }) => {
		...
		if (provider !== "auto" && selectedProvider && TOKENS_USED >= selectedProvider.max_tokens) {
			set.status = 400;
			return {
				ok: false,
							...
							while (true) {
								...
								for (const line of lines) {
									...
									if (line.includes("exceeded")) {
										console.error("Token limit exceeded during stream:", line);
										controller.enqueue(
											JSON.stringify({
												ok: false,
